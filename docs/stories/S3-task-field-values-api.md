...
- **Per-field multiselect via echo `c.Bind` — verified, closed.** The final fix wave exercised per-field `POST` and `PUT` with multiselect array bodies (`{"value":["red","blue"]}` / `{"value":["red"]}`) against the test instance: both bind host-side via echo and pass `writeValue`'s `raw.([]interface{})` assert under yaegi, returning native arrays with the child rows confirmed in sqlite. No `json.NewDecoder` swap was needed.
...
- **Both reads and writes return 403 (not 404) for a nonexistent task:** the `CanX` guards (`Task.CanRead`/`CanCreate`/`CanUpdate`/`CanDelete`) map `ErrTaskDoesNotExist` to `(false, nil)` — not-found ≡ no access under the Task-3 `CanX` contract — so GET, POST, and PUT all 403 on a bad task id, before the write path's exists-check or `writeValue` runs. The 404 (`ErrCustomFieldValueNotFound`) fires only when the task exists and is accessible but the specific field has no value row. Verified in the final fix wave (GET/POST/PUT on task 9999 → 403; PUT on an existing task with no value row → 404). Fix, if ever desired: a plugin-side task existence check (e.g. `models.GetTaskByIDSimple`) before delegating to the `CanX` guard, surfacing 404 on a not-found *task* (distinct from the value-not-found 404). Left as a documented behavior choice.
