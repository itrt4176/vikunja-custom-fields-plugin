...
The whitelist is consumed by any management surface: the field definition API (S2) and the temporary management UI (S6) both check it before allowing field-definition changes.
...
- **Must come before:** S2 (Field Definition API), S6 (Management UI)
...
3. A user not on the whitelist is denied field-definition operations (checked by S2/S6).
...
6. The whitelist is read from config and shared with both the field definition API (S2) and the management UI (S6).

## Scope

**In scope:**
- Config schema for the management whitelist
- Reading and exposing the whitelist to field-definition management (S2, S6)
- Clear error logging for malformed entries

**Out of scope:**
- Field definition CRUD — S2
- The management UI that consumes the whitelist — S6
